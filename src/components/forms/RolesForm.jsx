...
import { createRole, updateRole } from "../../DAL/roles";
...
const CreateRoleModal = ({ open, onClose, isEditing, data }) => {
    ...
    switch (field) {
      case 'name':
        ...
        if (value.trim().length < VALIDATION_RULES.NAME_MIN_LENGTH) {
          return `Role name must be at least ${VALIDATION_RULES.NAME_MIN_LENGTH} characters`;
        }
        if (value.trim().length > VALIDATION_RULES.NAME_MAX_LENGTH) {
          return `Role name must be less than ${VALIDATION_RULES.NAME_MAX_LENGTH} characters`;
        }
        return "";
      default:
        return "";
  ...
  return (
    <GenericFormModal
      ...
      title={`${isEditing ? 'Update' : 'Create'} Role`}
      maxWidth="sm"
    >
      <Stack spacing={3}>
