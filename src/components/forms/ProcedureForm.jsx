...
const CreateProcedureModal = ({ open, onClose, isEditing, data }) => {
    ...
    switch (field) {
      case 'name':
        ...
        if (value.trim().length < VALIDATION_RULES.NAME_MIN_LENGTH) {
          return `Procedure name must be at least ${VALIDATION_RULES.NAME_MIN_LENGTH} characters`;
        }
        if (value.trim().length > VALIDATION_RULES.NAME_MAX_LENGTH) {
          return `Procedure name must be less than ${VALIDATION_RULES.NAME_MAX_LENGTH} characters`;
        }
        return "";
      default:
        return "";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = isEditing
        ? await updateProcedure(data?.id, payload)
      ...
      if (res?.code && res.code !== 200 && res.code !== 201) {
        ...
        if (res.code === 422) {
          ...
          errorMessage = "A procedure with this name already exists";
      ...
      enqueueSnackbar(
        `Procedure ${isEditing ? 'updated' : 'created'} successfully!`,
        { variant: "success" }
      );
      resetForm();
      onClose();
  ...
  return (
    <GenericFormModal
      ...
      title={`${isEditing ? 'Update' : 'Create'} Procedure`}
      maxWidth="sm"
    >
      <Stack spacing={3}>
