...
- **Permissions**: 
  - `view` permission for `AgentDashboard` module (required)
...
- **Permission-based fields**: `total_incentive` is only included if user has the `total_incentive` permission for `AgentDashboard` module.
...
**Note:** The `total_incentive` field in the `cards` object is only included if the authenticated user has the `total_incentive` permission for the `AgentDashboard` module. Users without this permission will not see this field in the response.
